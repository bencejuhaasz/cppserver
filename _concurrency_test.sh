...
"$SERVER_BIN" --io-heavy > "$LOG_FILE" 2>&1 &
PID=$!
...
for i in $(seq 1 20); do
  if curl -sS --connect-timeout 1 "http://127.0.0.1:$PORT/" >/dev/null 2>&1; then
...
CONCURRENCY=400
echo "Running $CONCURRENCY parallel requests against http://127.0.0.1:$PORT/"
rm -f "$RESULTS_FILE"
seq 1 $CONCURRENCY | xargs -n1 -P$CONCURRENCY -I{} bash -c \
  'out=$(curl -sS -o /dev/null -w "code:%{http_code} time:%{time_total}" "http://127.0.0.1:$PORT/" 2>/dev/null); ec=$?; if [ "$ec" -ne 0 ]; then printf "id:%s error:exit=%d\n" "{}" "$ec"; else printf "id:%s %s\n" "{}" "$out"; fi' \
  >> "$RESULTS_FILE"
